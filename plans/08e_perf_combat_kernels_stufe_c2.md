...
> Panik/Knockback-Decay), danach **Fortsetzungsrunde „C3"** (Scan-Cache +
> Index-Scoring, Direct-Step-Hold, Group-Pass, C2.5). Peak-Block
> **59,1 → 44,1 ms (−25 %)**, Marschphase −27…−33 %; Suite 2697 grün.
> In-Game nach C2: 15–20 FPS (Nutzertest) — **30-FPS-Ziel noch nicht
> erreicht**; Restposten unten. Details: PROGRESS.md „Phase 8.e".
> Abweichung zu unten: Designproblem Nr. 1 wurde mit **Option 2
> (Generation-Handles)** gelöst, nicht Option 1 — Ranged-Angreifer
...
## Fortsetzung „C3" (umgesetzt 2026-07-23, zweite Runde — Peak 46,6 → 44,1 ms)

Die vier Punkte der Nachmessung, alle umgesetzt (Details PROGRESS.md „C3"):
1. **Block-Scan-Cache** (verhaltensschonend, Balance-Vorgabe): Kandidaten-
   Sammlung einmal pro 2×2-Zellblock × Stamm × ~0,2 s (Hit-Rate ~95 %),
   Konsumenten filtern exakt (Radius/Liveness) und **scoren über die
   SoA-Arrays** (Index-API + Distanz-Sortierung mit Early-Exit) — die
   Property-Reads pro Kandidat waren der eigentliche Scan-Fresser, nicht die
   Sammlung.
2. **Direct-Step-Chase-Hold** (`HOLD_CHASE_DIRECT`): Verfolgung ≤ 2,5 m läuft
   im Kernel gegen die neue flache `NavGrid.walkable_map`; deckt auch das
   Melee-Range-Pendeln ab (Drop bei d ≤ 1,2 → Strike, Re-Entry beim Pendeln).
3. **Combat-Group-Pass**: der Voll-Prune jeder Gruppe lief jeden Tick
   (~540 Gruppen ≈ 5–6 ms) — echte Mutationen sind event-getrieben, der
   Sweep fängt nur Leichen/Leaks → gestaffelt auf 1/8 der Gruppen pro Tick
   (`is_alive_light` dazwischen). Groups-Anteil ~5,5 → ~2,5 ms.
4. **C2.5**: `_claimed_by_peer` von O(Stammes-Prediger) auf O(1)
   (Reverse-Claim-Feld am Ziel, rückvalidiert) + CAST-Stand-Hold
...
Weitere Ideen (abnehmender Ertrag): Kernel-/Separation-Fan-out auf Threads
(SoA ist jetzt der Snapshot — Stufe-B-Blocker weg), GDExtension/C++ für die
Kernels, Feuerkrieger-Schusspfad, Zauber-Nachwirkungen (Brand/Panik).
