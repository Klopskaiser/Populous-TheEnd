...
- [ ] **Phase 10g** — [10g_ai_logistics.md](10g_ai_logistics.md): KI-Logistik in sieben unabhängig lieferbaren Teilen, alle aus Nutzertests. **Teil 6 ist umgesetzt** (Commit `3dd9572`): Reinkarnationsplatz ist **kein Angriffsziel mehr** — `is_assailable_by_units` → `is_attackable`, sieben ungeprüfte Zielpfade geschlossen (Katapult-/Feuerramme-Befehl, deren **Auto-Zielsuche** und ihre Zielgültigkeit, Luftschiff, KI-Angriffsziel + KI-Zauberheuristik ×2), 8 Tests / 16 Zusicherungen, Regel in CLAUDE.md §7; Nebenwirkung: die KI marschiert jetzt auf die **Anhänger** und verfolgt damit erstmals die 10d-Siegkette. Offen: **(7)** Bauordnung — Wohnraum und Ausbildung **vor** Werkstätten (der Zusatz-Lager-Zweig stand ganz am Ende, hinter bis zu 12 Werkstätten ⇒ nie mehr als 1 Trainingsgebäude je Art), Lagerzahl aus dem **Bravestrom** statt aus der Hüttenzahl, und Rally Points der Hütten auf die Ausbildungszentren (`Hut._spawn_brave` → `rally_training_building` kann das seit 5d, die KI setzt **nirgends** einen Rally Point); **(1)** aktive Bauarbeiter-Zuweisung mit Budget (die KI rief `order_build` **nie** — Baustellen bis 40 Zellen draußen sahen niemanden) plus Versorgungs-Tor vor neuen Bauplätzen; **(2)** Erreichbarkeit als *eine* Wahrheit (Inselvergleich am Anlaufpunkt statt A\* zur Plotmitte, `Building.approach_cell_for`, Bann mit Ablauf, Nachkontrolle bestehender Baustellen); **(3)** Fahrzeuge werden bemannt (Militär zuerst) und blockieren die Werkstatt nicht mehr dauerhaft; **(4)** gezielter Holznachschub (KI-intern), Werkstätten in der Bedarfsrechnung, Holzregal im Absorptionsradius der Werkstatt; **(5)** Arena-Maß + zwei Baumuster für kleine/große Karten (Insel mit 4 Stämmen: Nachbarbasis 36 Zellen bei `DEFEND_RADIUS` 32 ⇒ Bedrohung dauerhaft, Holzwirtschaft aus, nie ein echter Angriff, Braves als Miliz). *(Plan 2026-08-04; setzt 10f voraus)*
