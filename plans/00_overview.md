...
- [ ] **Phase 10g** — [10g_ai_logistics.md](10g_ai_logistics.md): KI-Logistik in sechs unabhängig lieferbaren Teilen, alle aus Nutzertests: **(6)** Reinkarnationsplatz ist **kein Angriffsziel mehr** — `is_assailable_by_units` → `is_attackable`, vier ungeprüfte Zielpfade geschlossen (Katapult-/Feuerramme-Befehl **und** Auto-Zielsuche, Luftschiff, KI-Zauber- und Angriffszielwahl); **(1)** aktive Bauarbeiter-Zuweisung mit Budget (die KI rief `order_build` **nie** — Baustellen bis 40 Zellen draußen sahen niemanden) plus Versorgungs-Tor vor neuen Bauplätzen; **(2)** Erreichbarkeit als *eine* Wahrheit (Inselvergleich am Anlaufpunkt statt A\* zur Plotmitte, `Building.approach_cell_for`, Bann mit Ablauf, Nachkontrolle bestehender Baustellen); **(3)** Fahrzeuge werden bemannt (Militär zuerst) und blockieren die Werkstatt nicht mehr dauerhaft; **(4)** gezielter Holznachschub (KI-intern), Werkstätten in der Bedarfsrechnung, Holzregal im Absorptionsradius der Werkstatt; **(5)** Arena-Maß + zwei Baumuster für kleine/große Karten (Insel mit 4 Stämmen: Nachbarbasis 36 Zellen bei `DEFEND_RADIUS` 32 ⇒ Bedrohung dauerhaft, Holzwirtschaft aus, nie ein echter Angriff, Braves als Miliz). *(Plan 2026-08-04; setzt 10f voraus)*
