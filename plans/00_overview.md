...
- [x] **Phase 10i** — [10i_construction_bugfix_combat_balance.md](10i_construction_bugfix_combat_balance.md): Zwei Bugs aus dem Nutzertest + drei Balance-Mechaniken. **(F1–F7)** Baustellen, an denen nichts passiert — Ursache am Code belegt: `delivery_point()` ist **nicht persistiert** und springt (Ringsuche nimmt die *erste* Zelle in Rasterordnung), womit Holzbuchung, `wood_incoming`, Lieferziel, Erstattung *und* `approach_island` gegeneinander laufen → endlose Holzschleife (der „riesige Stapel"), `progress_cap` bleibt 0, Arbeiter hämmern ins Leere, `approach_island == -1` legt Rekrutierung **und** manuelle Zuweisung still. Dazu Eingangs-Planierzelle optional, Anlaufpunkt-Prüfung in `can_place_at` (nutzt das mit 10g extrahierte `approach_cell_for`), Fluchtweg für eingeschlossene Braves, Planierkante abflachen, Bauverfall härten. **(F8)** `State.TRAIN` fehlt in **beiden** `_anim_base()` → Braves laufen in der **Idle**-Animation zum Trainingslager. **(Balance)** Kämpfende feindliche Schamanin **stört die Predigt** (6 m — Gegenmittel gegen die im Balance-Labor belegte Prediger-Dominanz: 20 Prediger bekehren zeitgleiche 33 Krieger verlustfrei), Feuerkrieger-Feuerball erhält **Flächenschaden** (2 m, 50 %, nur Feinde, kein Rückstoß — sie teilen 36 % des Pools aus und töten fast nichts), **Anhebe-Chance skaliert invers mit den HP** (4 % → 12 %), Balance-Labor besetzt Luftschiffe endlich mit Feuerkriegern *(umgesetzt 2026-08-05 in drei Commits, Suite 4010 grün, Ladecheck sauber. **F6 (Planierkante) wurde nach Messung VERWORFEN** — es kostete die KI ein Drittel bis die Hälfte ihrer Bevölkerung (pop@300 s bergpass 242/244 ohne, 158/159 mit, per Bisektion auf F6 allein isoliert: F3 allein 241), und auch eine Klemmung gegen `MAX_SLOPE` half nicht; der Ringgraben ist über F5/F3/F4 abgedeckt. F4 war entgegen dem Plan kein Einzeiler (`can_place_at` hatte keine `orientation`). Mittendrin kamen zwei Nutzerreports dazu: die **KI konnte kein Gebäude zerstören** (der Attack-Move-Refresh löschte alle 4 Ticks jeden begonnenen Sturm — ohne Fix `hp 120 -> 120 after 60 s`) und der **Feuerstrahl der Feuerramme überlebte ihre Explosion**. Gemessen: Feuerkrieger schlagen ihren Schaden jetzt in Kills um (`feuerkrieger_vs_prediger` Effizienz 0,00 → 6,00), der Splash macht den Stresstest sogar **billiger** (Ø 100 → 53 ms, weil er tötet), die Prediger-Störung wirkt **lokal** und dreht das 20-gegen-20 nicht. **Manuelle Prüfung ausstehend** — Details PROGRESS.md)*
- [x] **Phase 10j** — [10j_disc_world.md](10j_disc_world.md): **Scheibenwelt im Weltall** — alle Karten rund, Sternenhimmel statt flacher Hintergrundfarbe, über den Rand Geschleuderte stürzen ins All und sterben **lautlos offscreen** (Ausnahme Schamanin: hörbar, Mana-Bonus, Respawn), Wasserfall am Rand wo Wasser liegt (nackte Felskante sonst). Hebel ist eine Scheibenmaske in `TerrainData.is_walkable()` — über `NavGrid.update_region()` als einzigen Solidity-Writer zieht das A-Stern, PathWorker, Inseln, Baum-/Einheiten-Spawns und `can_place_at` automatisch mit. **Löst den Widerspruch zu `95b0e73` auf:** die unsichtbare Mauer fällt weg, weil der eigentliche Bug war, dass `get_height()` draußen die Randhöhe fortsetzt — es gibt künftig einfach keinen Boden mehr. **Blocker:** die Startanker von Seenland und Plateau liegen in den Ecken, also außerhalb der Scheibe — drei der vier Karten brauchen echten Neuentwurf *(umgesetzt 2026-08-05, Suite 4147 grün, Ladecheck sauber. **Vier Planannahmen am Code widerlegt:** (K1) die Eckanker lagen mit 0,4525·size INNERHALB der Scheibe — kein Kartenneuentwurf nötig, nur die Ankerfraktion 0,18 → 0,22 für 15 statt 7 Zellen Randabstand; (K2) Terrain-Zauber können die Scheibe strukturell nicht vergrößern, weil die Maske IN `is_walkable()` sitzt — sechs geplante Edits entfielen, ein Test belegt es; (K3) der Plan nannte kein Rand-Mesh, ohne das die Scheibe papierdünn ist → `TerrainRim` (Felsband + geschlossene Unterseite, Nutzerentscheidung); (K4) `_snap_to_ground()` war entgegen seinem Kommentar **nie** der unumgehbare Backstop (die C2-Kernels schreiben `position` direkt) — der Kommentar ist berichtigt. **Nutzervorgaben:** alle Karten ×1,128 (144/144/288/288), damit die Scheibenfläche der alten Quadratfläche entspricht (99,4 %); endliche Scheibe mit Unterseite. **KI:** `AI_CRAMPED_ARENA` 60 → 68 mitskaliert, damit jede Karte ihre Einstufung behält; radiale Snap-Klemmung in allen DREI `nearest_walkable_cell`-Kopien (auf 288 lag die Ecke 59,6 > `MAX_SNAP_RADIUS` 32 vom Land — das schaltete die Bauplatz-Nachkontrolle still ab); Void kostet kein Suchbudget. **Gemessen:** Suite 161,7 → 142,6 s (21,5 % der Zellen dauerhaft solid), `benchmark_earlygame` pop-identisch (84/107/133/159 gegen 84/106/131/160) bei weniger Plot-Zellen. **Manuelle Prüfung ausstehend** — Details PROGRESS.md)*
- [x] **Phase 10k** — [10k_mana_spells.md](10k_mana_spells.md): **Magie und Wirtschaft neu tariert** — **(1+2)** Manaerzeugung gedaempft (ab 100 Anhaengern ~1/4 je Kopf; 500 = genau 2x, 1000 = 3,197x von 100; die Konstanten fallen aus den Zielwerten heraus, `400/10000 = 0,04`), Grundrate 0,11, Foerster-Unterhalt 1,5 -> 0,6, **Ausbildung kostet 0,6 Mana/s** (nur mit Trainee drin; Fahrzeugbau und Huetten-Bevoelkerung kosten NICHTS, die Buchung sitzt bewusst auf `TrainingBuilding`), alle 11 Zauberkosten und 6 Reichweiten neu — Gesamtspeicher 2460 -> 10 310 Mana. **(7)** Ladeanzeige nach Original-Vorbild: blauer Fortschritt, goldener Ratenbalken dahinter, logarithmische Umlaufzeit, Restzeit als Zahl ab 30 s. **(KI)** schaltet Zauber ab — eine Regel plus Boden, die Verschwendungsvermeidung faellt aus der Konstruktion heraus; **gemessen 22 -> 73/83 KI-Casts** (Faktor 3,3), Messinstrument `dbg_casts` mitgebaut. **(6)** neuer Zauber **Hypnose** (210 Mana, 3 Ladungen, 4x4 m, 30 s Fremdkontrolle, nur die Schamanin immun; die letzten Einheiten zu hypnotisieren beendet den Stamm — Nutzerentscheidung, die eine zweite Zugehoerigkeitsliste erspart). **(5)** Vulkan mit echtem **Krater** (Radius 7, Rand bei 0,55xR, Mulde 1,5 m, EIN Schwall, danach Gipfelplateau). **(3)** Feuerball 20/10 und **wirbelt 4/2 m hoch**, verfolgt sein Ziel. **(4)** Feuerregen als **Dauerregen** (20 s, Zufallsintervalle Oe 0,3 s, Brand, 20 HP Gebaeudeschaden je Ball, Panikring) *(umgesetzt 2026-08-07 in sechs Commits, Suite 4677 gruen (Start 4177), Ladecheck sauber. **`benchmark_stress` war fuer den Feuerregen untauglich** — es wirkt gar keinen, was nur auffiel, weil die Ueberlebendenzahlen identisch blieben; neu `benchmark_firestorm.gd` nach dem Muster von `benchmark_lava`: proj-Phase 0,58 -> 2,63/2,73 ms bei 11 statt 3 Projektilen. Neun bestehende Tests kodierten alte Werte (60/30 Schaden, 8 Baelle, Kegelspitze, zweimal "elf Zauber"). **Manuelle Pruefung ausstehend**, insbesondere die Vulkanform — die einzige offene Entscheidung des Plans, reine Balance-Werte. Details PROGRESS.md)*
