...
- [ ] **Phase 10a** — [10a_water_animations_drowning.md](10a_water_animations_drowning.md): Undurchsichtiges Populous-Wasser, neue Animationen `airborne`/`drown`, echte Ertrink-Mechanik (Einheiten stoppen nicht mehr an der Wasserkante), 2D-Feuereffekt der Feuerramme, Politur des Gebäude-Versinkens *(umgesetzt 2026-08-02: Suite 2977 grün, Ladecheck sauber; **manuelle Prüfung durch den Nutzer ausstehend** — Details PROGRESS.md)*
- [ ] **Phase 10b** — [10b_sound_priority_spell_voices.md](10b_sound_priority_spell_voices.md): Zauberformel der Schamanin beim Cast-Beginn + Effektsound beim Einschlag (Cast-Zeit 1,0 s), Sound-Prioritätensystem (1 Zauberformeln/Schamanentod, 2 Fahrzeug-/Zeppelintode, 3 Rest) inkl. Behebung des Slot-Verwurf-Bugs im AudioManager *(umgesetzt 2026-08-03: Suite 3068 grün, Ladecheck sauber; **manuelle Prüfung ausstehend und ohne Audiodateien nur eingeschränkt möglich** — Details PROGRESS.md)*
- [x] **Phase 10c** — [10c_lava_knockback_lift.md](10c_lava_knockback_lift.md): Lava rot, zäh, geländefolgend (gemeinsames Modell `LavaCommon`, Lebensdauer in `Balance`, Vulkan 4 → 2 Stöße, Erdbeben-Lava an der oberen Bruchkante) mit verbindlicher Perf-Abnahme; Rückstoß-/Anhebe-System für Feuerball, Feuerregen, Blitz und Feuerkrieger *(umgesetzt 2026-08-03: Suite 3131 grün, Ladecheck sauber. **Perf:** Lava-Code isoliert gemessen **44 % billiger** (neuer `tests/benchmark_lava.gd`), `proj`-Kriterien erfüllt (Ø 0,92 ms, schlechtester Block 2,10 ms) — aber Gesamt-Ø des Stresstests 16,9 → 27,9 ms; Treiber ist nachweislich der **Feuerkrieger-Lift** (Pfadzeit 1,4 → 10,8 s), nicht die Lava.)* *(**Nachtrag 2026-08-03** nach Nutzertest „Lift zu extrem": Flughöhen-Deckel `LIFT_MAX_HEIGHT = 6 m` (Abwurf-Budget + Positionskappe), Luftbonus 4,0 → 2,0, und **Ragdoll** für in der Luft bereits tödlich getroffene Einheiten (`Unit.doomed`: Kampfgruppe/Befehle/Ziel-Slot sofort frei, nur noch Flugbogen + Purzeln). Damit ist die Pfad-Explosion weg — **57 722 Pfade gegen 65 237 im Vor-10c-Stand**, Laufzeit im verschränkten A/B auf Parität; die zuvor gemeldeten „+65 %" waren Messartefakt (Maschinenlast). Suite 3150 grün. **Offen:** manuelle Prüfung — Details PROGRESS.md)*
- [ ] **Phase 10d** — [10d_buildings_win_conditions_ui.md](10d_buildings_win_conditions_ui.md): Siegbedingungen (Schamanenkreis unverwundbar + Selbstzerstörung, Ausscheiden des Stammes), Gebäudeabriss per `Entf` (100 %/75 % Erstattung), Bauverfall nach 2 min, Erreichbarkeits-Gate für Bauarbeiter, Bau mit selektierten Braves, UI-Bugfixes (Angriffs-Cursor, bekehrte Einheiten)
- [ ] **Phase 10e** — [10e_wood_area_ai_overhaul.md](10e_wood_area_ai_overhaul.md): Holzfäll-Rechteck für Braves (`B`, `Shift+B` = alle Hütten) inkl. weißem Bestätigungs-Blinken um beauftragte Bäume, und KI-Überarbeitung (Tick-Cache, aktive Holzlogistik mit Fäll-Trupps und vorgeschobenen Lagern, Bauplatz-Abstände/Ausrichtung, mehr Prediger, skalierende Fahrzeuglimits, Spätspiel-Skalierung bis ~1000 Bevölkerung)
